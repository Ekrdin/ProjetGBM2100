...
/*
    ...
    parameters :  uint8_t dev_addr, adress of the I2C
               uint8_t reg_addr, adress of the register we want to write in
               uint8_t *data, data we want to write in
               uint16_t lenght, size of the data we want to write
    This function is used to write multiple bytes of data, on a specific register of the I2C, as the register adress 
have an automatic incrementation of 1, those data are consecutive on the register. 
The process use is the one explained on the BMI160 DataSheet p96.
*/
 void BMI160BurstWrite ( uint8_t dev_addr, uint8_t reg_addr, uint8_t *data,uint16_t lenght);
